...
let private internalEncode(input: ReadOnlySpan<byte>, output: Span<char>, numZeroes: int) : int =
...
let Encode(bytes: byte[]) : string =
    ...
    let numCharsWritten = internalEncode(ReadOnlySpan<byte>(bytes), (Span<char>(output)), numZeroes)
